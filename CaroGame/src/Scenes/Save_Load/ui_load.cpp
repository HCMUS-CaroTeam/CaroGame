...
static void ActionRenameSave() {
    string oldKey = gSelectedKey;
    string newKey = string(gRenameBuffer);

    if (newKey.empty()) {
        gRenameStatusMsg = "NAME CANNOT BE EMPTY!";
        return;
    }
    if (gameSaves.count(newKey) && newKey != oldKey) {
        gRenameStatusMsg = "NAME ALREADY EXISTS!";
        return;
    }
...
)
{
    ...
    if (gShowRenameOverlay) {
        ...
        if (confirmClicked) {
            PlayMenuClick(audio, settings);
            gNotificationMessage = "RENAMED GAME \"" + gSelectedKey + "\" TO GAME \"" + gRenameBuffer + "\" SUCCESSFULLY!";
            ActionRenameSave();
    ...
    if (gShowDeleteConfirm) {
        ...
        if (yesClicked) {
            PlayMenuClick(audio, settings);
			gNotificationMessage = "DELETED \"" + string(current().nameGame) + "\" SUCCESSFULLY!";
            ActionDeleteSave();
