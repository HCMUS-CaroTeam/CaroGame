...
void DrawPlayUI(Font fontTitle, Font fontSmall, const MouseState &mouse,
  ...
  if (current().result == RESULT_X_WINS)
    DrawCenteredText(fontSmall, "WINNER: X  |  R: RESET  |  ESC: PAUSE", 70.0f,
                     24.0f, Color{120, 220, 255, 255});
  else if (current().result == RESULT_O_WINS)
    DrawCenteredText(fontSmall, "WINNER: O  |  R: RESET  |  ESC: PAUSE", 70.0f,
                     24.0f, Color{255, 150, 200, 255});
  else if (current().result == RESULT_DRAW)
    DrawCenteredText(fontSmall, "DRAW  |  R: RESET  |  ESC: PAUSE", 70.0f,
                     24.0f, Color{240, 225, 225, 220});
  else if (current().turn == CELL_X)
    DrawCenteredText(
        fontSmall, "TURN: X  |  5 IN A ROW TO WIN  |  R: RESET  |  ESC: PAUSE",
        70.0f, 24.0f, Color{120, 220, 255, 255});
  else
    DrawCenteredText(
        fontSmall, "TURN: O  |  5 IN A ROW TO WIN  |  R: RESET  |  ESC: PAUSE",
        70.0f, 24.0f, Color{255, 150, 200, 255});

  // --- HIỂN THỊ COUNTDOWN TIMER (Tournament / PVE) ---
  float timeLeft = GetTimeLeft();
  if (timeLeft >= 0.0f && current().result == RESULT_ONGOING) {
    // Hiển thị thời gian còn lại ở góc trên bên phải bàn cờ
    char timerBuf[32];
    int seconds =
    ...
    float timerX = BOARD_START_X + BOARD_PIXEL_SIZE + 20.0f;
    float timerY = BOARD_START_Y + 10.0f;
    DrawTextEx(fontSmall, timerBuf, Vector2{timerX, timerY}, 28.0f, 1.0f,
               timerColor);
  }
