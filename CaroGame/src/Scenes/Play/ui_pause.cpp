...
)
{
    ...
    for (int i = 0; i < gPauseButtonCount; ++i)
    {
        ...
        if (hovered && mouse.leftPressed)
        {
            ...
            case PAUSE_BTN_SAVE:
                ClosePause();
				if (current().nameGame[0] == '\0' || strcmp(current().nameGame, "AutoSave") == 0) { // Nếu chưa có tên game hoặc đang là AutoSave thì bắt buộc phải nhập tên mới, không cho phép lưu đè
