...
static constexpr float BOARD_START_Y = 235.0f; // Vị trí Y cố định để bàn cờ nằm ở phần trên màn hình
...
struct DataGame {
	...
	int winLine[MAX_WIN_LINE][2];     // Mảng lưu tọa độ của các ô tạo thành đường thắng, chỉ có giá trị khi result là RESULT_X_WINS hoặc RESULT_O_WINS
	int winLineCount;                 // Số ô thực tế trong đường thắng (có thể > WIN_LENGTH ở Classic mode)
};
#pragma pack(pop)
