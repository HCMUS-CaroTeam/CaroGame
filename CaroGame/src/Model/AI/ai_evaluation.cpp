...
long long EvaluatePositionMedium(int r, int c, int piece) {
    ...
    for (int dir = 0; dir < 4; dir++) {
        ...
        if (blocks == 2 && count < 5) continue;
        if (count >= 5) totalScore += MEDIUM_SCORES[WIN];
        else if (count == 4) totalScore += (blocks == 0) ? MEDIUM_SCORES[OPEN_4] : MEDIUM_SCORES[BLOCKED_4];
        else if (count == 3) totalScore += (blocks == 0) ? MEDIUM_SCORES[OPEN_3] : MEDIUM_SCORES[BLOCKED_3];
        else if (count == 2) totalScore += (blocks == 0) ? MEDIUM_SCORES[OPEN_2] : MEDIUM_SCORES[BLOCKED_2];
        else if (count == 1) totalScore += (blocks == 0) ? MEDIUM_SCORES[OPEN_1] : MEDIUM_SCORES[BLOCKED_1];
    }
    return totalScore;
}

long long EvaluatePositionHard(int r, int c, int piece, bool isBoardEval) {
    long long totalScore = 0;
    int dx[] = {1, 0, 1, 1};
    int dy[] = {0, 1, 1, -1};
    ...
    for (int dir = 0; dir < 4; dir++) {
        ...
        if (isBoardEval) {
            ...
            if (prev_r >= 0 && prev_r < BOARD_SIZE && prev_c >= 0 &&
                prev_c < BOARD_SIZE) {
                // Nếu ô ngay phía sau là cờ của mình -> Chuỗi này đã được đếm từ đầu mút -> Bỏ qua!
                if (current().board[prev_r][prev_c] == piece)
                    continue;

                // Mở rộng: Kiểm tra lùi 2 ô đề phòng chuỗi bị gãy 1 ô (Vá triệt để lạm phát cờ gãy)
                int prev2_r = r - 2 * dx[dir];
                int prev2_c = c - 2 * dy[dir];
                if (current().board[prev_r][prev_c] == 0 && prev2_r >= 0 &&
                    prev2_r < BOARD_SIZE && prev2_c >= 0 && prev2_c < BOARD_SIZE &&
                    current().board[prev2_r][prev2_c] == piece) {
                    continue;
                }
            }
        }
        ...
        int totalGaps = 0; // Biến mới: Theo dõi tổng số ô trống của cả hướng TỚI và LÙI
        ...
        while (true) {
            ...
            if (current().board[nr][nc] == piece) {
                ...
                if (gaps == 0 && next_r >= 0 && next_r < BOARD_SIZE && next_c >= 0 && next_c < BOARD_SIZE &&
                    current().board[next_r][next_c] == piece) { // Đằng sau ô trống LÀ CỜ CỦA MÌNH
        ...
        while (true) {
            ...
            if (current().board[nr][nc] == piece) {
                ...
                if (gaps == 0 && next_r >= 0 && next_r < BOARD_SIZE && next_c >= 0 && next_c < BOARD_SIZE &&
                    current().board[next_r][next_c] == piece) {
                    gaps++;
                    totalGaps++;
                    step++;
                }
        ...
        if (type != PATTERN_NONE) totalScore += HARD_SCORES[type];
    }
    return totalScore;
}
